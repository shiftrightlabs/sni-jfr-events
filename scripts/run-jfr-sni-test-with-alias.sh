...
# Using client hostname: kafka-with-alias.domaina.com
...
SERVER_HOSTNAME="kafka-broker.domaina.com"
CLIENT_HOSTNAME="kafka-with-alias.domaina.com"
PORT="9093"
JFC_FILE="$KAFKA_PLAYGROUND/custom-tls.jfc"
JFR_OUTPUT="kafka-jfr-test-alias-$(date +%Y%m%d-%H%M%S).jfr"
EVIDENCE_FILE="jfr-test-evidence-alias-$(date +%Y%m%d-%H%M%S).txt"
...
check_prerequisites() {
    ...
    # Check /etc/hosts for server
    if grep -q "$SERVER_HOSTNAME" /etc/hosts; then
        echo "✅ $SERVER_HOSTNAME found in /etc/hosts"
    else
        echo -e "${RED}❌ $SERVER_HOSTNAME not found in /etc/hosts${NC}"
        echo "Please add: echo '127.0.0.1    $SERVER_HOSTNAME' | sudo tee -a /etc/hosts"
        exit 1
    fi

    # Check /etc/hosts for client alias
    if grep -q "$CLIENT_HOSTNAME" /etc/hosts; then
        echo "✅ $CLIENT_HOSTNAME found in /etc/hosts"
    else
        echo -e "${RED}❌ $CLIENT_HOSTNAME not found in /etc/hosts${NC}"
        echo "Please add: echo '127.0.0.1    $CLIENT_HOSTNAME' | sudo tee -a /etc/hosts"
        exit 1
    fi
    ...
    if grep -q "advertised.listeners=SSL://$SERVER_HOSTNAME:$PORT" $KAFKA_HOME/etc/kafka/server.properties; then
...
start_services() {
    ...
    KAFKA_OPTS="$JFR_OPTS" $KAFKA_HOME/bin/kafka-server-start \
        $KAFKA_HOME/etc/kafka/server.properties > kafka-jfr-alias.log 2>&1 &
    KAFKA_PID=$!
...
generate_traffic() {
    ...
    echo "Server Hostname: $SERVER_HOSTNAME" >> $EVIDENCE_FILE
    echo "Client Hostname (alias): $CLIENT_HOSTNAME" >> $EVIDENCE_FILE
    echo "" >> $EVIDENCE_FILE

    # Generate multiple connections using the alias hostname
    for i in {1..5}; do
        echo -e "${YELLOW}Connection attempt $i using $CLIENT_HOSTNAME...${NC}"
    ...
    echo "✅ Generated 5 TLS connections using client hostname: $CLIENT_HOSTNAME"
}
...
verify_certificates() {
    ...
    if [ "$CN" = "$SERVER_HOSTNAME" ]; then
        echo "✅ Certificate CN matches server hostname"
    else
...
generate_summary() {
    ...
    SNI_CONSUMED=$(grep -c "Consumed extension: server_name" kafka-jfr-alias.log 2>/dev/null || echo "0")

    # Secondary checks - look for the client hostname in SNI
    SNI_SENT_CLIENT=$(grep -c "type=host_name.*$CLIENT_HOSTNAME" $EVIDENCE_FILE || echo "0")
    SNI_SENT_SERVER=$(grep -c "type=host_name.*$SERVER_HOSTNAME" $EVIDENCE_FILE || echo "0")
    ...
    echo "- Client sent SNI with $CLIENT_HOSTNAME: $SNI_SENT_CLIENT times" | tee -a $EVIDENCE_FILE
    echo "- Client sent SNI with $SERVER_HOSTNAME: $SNI_SENT_SERVER times" | tee -a $EVIDENCE_FILE
    ...
    echo "   grep -q 'Consumed extension: server_name' kafka-jfr-alias.log && echo 'PASS' || echo 'FAIL'"
    echo ""
    echo "2. Check what hostname was sent in SNI:"
    echo "   grep 'type=host_name' $EVIDENCE_FILE"
    echo ""
    echo "3. Analyze JFR (if jfr command available):"
    echo "   jfr print --events jdk.TLSHandshake $JFR_OUTPUT | grep peerHost"
    echo ""
    echo "4. Check with jcmd:"
    echo "   jcmd $(pgrep -f kafka.Kafka) JFR.check"

    # PASS/FAIL determination based on server-side evidence
    if [ $SNI_CONSUMED -gt 0 ] && [ $SNI_SENT_CLIENT -gt 0 ]; then
        echo -e "\n${GREEN}✅ TEST PASSED${NC}" | tee -a $EVIDENCE_FILE
        echo "Server received SNI extension $SNI_CONSUMED times" | tee -a $EVIDENCE_FILE
        echo "Client sent SNI with hostname: $CLIENT_HOSTNAME" | tee -a $EVIDENCE_FILE
        echo "Certificate CN: $CN" | tee -a $EVIDENCE_FILE
    else
        ...
        if [ $SNI_SENT_CLIENT -eq 0 ]; then
            echo "Client did not send SNI with hostname: $CLIENT_HOSTNAME" | tee -a $EVIDENCE_FILE
        fi
        echo "Check $EVIDENCE_FILE and kafka-jfr-alias.log for details" | tee -a $EVIDENCE_FILE
    fi
}

# Main execution
main() {
    echo -e "${BLUE}Starting JFR SNI/CN Test with Client Alias...${NC}"
